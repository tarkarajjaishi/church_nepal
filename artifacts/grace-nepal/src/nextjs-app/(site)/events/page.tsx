...
function EventCard({ e, upcoming }: { e: any; upcoming?: boolean }) {
  const { lang } = useLang();
  const bs = e.date ? toBS(e.date, lang as "en" | "ne") : "";
...
export default function Events() {
  ...
  // @ts-ignore
  const categories = Array.from(new Set(events.map(e => (e as any).category).filter(Boolean)));

  // Filter events by selected category
  const filteredEvents = events.filter(event => {
    if (!selectedCategory) return true;
    return (event as any).category === selectedCategory;
  });
