...
pub async fn list(Db(pool): Db, Query(p): Query<Pagination>) -> Result<Json<Paginated<GalleryItem>>, AppError> {
    let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM gallery").fetch_one(&pool).await?;
    let rows = sqlx::query_as::<_, GalleryItem>("SELECT * FROM gallery ORDER BY created_at DESC LIMIT $1 OFFSET $2")
