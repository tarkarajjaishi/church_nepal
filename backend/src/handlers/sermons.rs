...
pub async fn update(
     ...
     let row = sqlx::query_as::<_, Sermon>(
         ...
         .bind(input.speaker.as_deref().map(|t| xss::sanitize_plain(t)).or(Some(existing.speaker.clone())))
         .bind(input.date.clone().or(Some(existing.date.clone())))
         .bind(input.duration.clone().or(Some(existing.duration.clone())))
         .bind(input.series.as_deref().map(|t| xss::sanitize_plain(t)).or(Some(existing.series.clone())))
         .bind(input.topic.as_deref().map(|t| xss::sanitize_plain(t)).or(Some(existing.topic.clone())))
         .bind(input.image.clone().or(Some(existing.image.clone())))
         .bind(input.description.as_deref().map(|t| xss::sanitize_html(t)).or(Some(existing.description.clone())))
