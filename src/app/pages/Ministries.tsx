            ...
            {isLoading ? (
              ...
              <Reveal key={m.id} delay={i * 0.05}>
                <Card className="group overflow-hidden h-full border-border/60 hover:shadow-xl transition-all gap-0 flex flex-col">
                  <Link to={`/ministries/${m.id}`} className="relative h-44 overflow-hidden block">
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <Button asChild size="sm" className="mt-4 bg-church-blue hover:bg-church-blue/90"><Link to="/contact">Join Ministry <ArrowRight className="size-4" /></Link></Button>
                  </div>
                </Card>
              </Reveal>
            ))}
