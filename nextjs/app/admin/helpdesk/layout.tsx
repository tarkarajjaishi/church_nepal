...
import {
  LayoutDashboard, Ticket, Inbox, AlertTriangle, BookOpen, Tags,
  ChevronLeft, Menu, X,
} from 'lucide-react'
...
const NAV = [
  {
    group: 'Queue',
    items: [
      { href: '/admin/helpdesk', label: 'Dashboard', icon: LayoutDashboard },
      { href: '/admin/helpdesk/tickets', label: 'All tickets', icon: Ticket },
      { href: '/admin/helpdesk/unassigned', label: 'Unassigned', icon: Inbox },
      { href: '/admin/helpdesk/breaching', label: 'Breaching SLA', icon: AlertTriangle },
    ],
  },
  {
    group: 'Self service',
    items: [{ href: '/admin/helpdesk/knowledge', label: 'Knowledge base', icon: BookOpen }],
  },
  {
    group: 'Configure',
    items: [{ href: '/admin/helpdesk/categories', label: 'Categories & SLA', icon: Tags }],
  },
]
...
export default function HelpdeskLayout({ children }: { children: React.ReactNode }) {
  ...
  const isActive = (href: string) =>
    href === '/admin/helpdesk' ? pathname === href : pathname.startsWith(href)

  const nav = (
    <nav className="space-y-6" aria-label="Help desk">
      {NAV.map(({ group, items }) => (
        <div key={group}>
    ...
    <div className="flex gap-6">
      <aside className="hidden lg:block w-56 shrink-0">
        <div className="sticky top-4 space-y-4">
          ...
          <div>
            <h2 className="px-3 font-semibold text-foreground">Help Desk</h2>
            <p className="px-3 text-xs text-muted-foreground mt-0.5">Requests &amp; repairs</p>
          </div>
          {nav}
        </div>
      </aside>
