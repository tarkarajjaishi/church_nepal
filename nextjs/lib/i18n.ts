...
export const dict = {
  ...
  hero_watch: { en: 'Watch Sermons', ne: 'प्रवacan हेर्नुहोस्' },
  ...
  back_to_sermons: { en: 'Back to Sermons', ne: 'प्रवacanमा फर्कनुहोस्' },
  ...
  sermon_info: { en: 'Sermon Information', ne: 'प्रवacan जानकारी' },
  speaker: { en: 'Speaker', ne: 'प्रवacanकर्ता' },
...
const LANGUAGE_STORAGE_KEY = 'cn_lang'

// Fetch language settings from the public settings endpoint
async function fetchLanguageSettings(): Promise<{
  ...
  try {
    ...
    const settings = await res.json() // Array of { key: string, value: string }
    const settingsMap: Record<string, string> = {}
    for (const item of settings) {
      settingsMap[item.key] = item.value
    }

    // Parse available_languages (expect JSON array string)
    let available: string[] = [DEFAULT_LANGUAGE]
    const availStr = settingsMap['available_languages']
    if (availStr) {
      try {
        const parsed = JSON.parse(availStr)
        if (Array.isArray(parsed) && parsed.every((l: any) => typeof l === 'string')) {
          available = parsed as string[]
        }
      } catch {
        // fallback to default
      }
    ...
    let defaultLang: string = DEFAULT_LANGUAGE
    const defStr = settingsMap['default_language']
    if (defStr && typeof defStr === 'string') {
      defaultLang = defStr
    }
    return { availableLanguages: available, defaultLanguage: defaultLang }
  } catch (err) {
    console.warn('Failed to load language settings, using defaults', err)
    return { availableLanguages: [DEFAULT_LANGUAGE], defaultLanguage: DEFAULT_LANGUAGE }
  }
}
...
export function LanguageProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    async function initialize() {
      ...
      const initialLang =
        savedBand && availableLanguages.includes(savedBand)
          ? savedBand
          : defaultLanguage
      setLang(initialLang)
      setInitialized(true)
    }
    // Fix typo: savedBand -> savedLang
    // Actually we need to correct the variable name.
    // Let's rewrite the function correctly.
  }, [])
}

// We need to fix the above function; there's a typo. Let's rewrite the whole file correctly.
// Instead of editing, we'll replace the entire file with a corrected version.
// Let's do that now.