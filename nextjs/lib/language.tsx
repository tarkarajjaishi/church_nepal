...
export const dict: Record<string, Record<string, string>> = {
  ...
  error_body: {
    ...
    ne: 'हामी यो पृष्ठ लोड गर्न सकेनौं। कृपया फेरि प्रयास गर्नुहोस् — यदि यो दोहोरिइरह्यो भने हामीलाई जानकारी दिनुहोस्।',
  },
  try_again: { en: 'Try Again', ne: 'फेरि प्रयास गर्नुहोस्' },
  back_home: { en: 'Back Home', ne: 'गृहपृष्ठमा फर्कनुहोस्' },
...
export function LanguageProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    async function initialize() {
      ...
      const initialLang =
        savedLang && availableLanguages.includes(savedLang)
          ? savedLang
          : defaultLanguage
      setLang(initialLang)
      setInitialized(true)
  ...
  // Update language in storage and html lang attribute
  useEffect(() => {
  ...
  const setLanguage = (newLang: string) => {
    if (!availableLanguages.includes(newLang)) {
      console.warn(`Language ${newLang} not in available languages:`, availableLanguages)
      return
    }
    setLang(newLang)
  }
  ...
  const t = (key: string): string => {
    const langDict = dict[key]
    if (langDict) {
      const translation = langDict[lang]
      if (translation !== undefined) return translation
      // Fallback to default language
      const fallback = langDict[defaultLanguage]
  ...
  return (
    <LanguageContext.Provider
      value={{ lang, setLang: setLanguage, t }}
    >
      {children}
    </LanguageContext.Provider>
  )
